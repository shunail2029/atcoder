...
double area(const Point &a, const Point &b, const Point &c) {
    double len_a = dis(b, c), len_b = dis(c, a), len_c = dis(a, b);
...
Point circumcenter(const Point &a, const Point &b, const Point &c) {
    double A = std::acos(dot(b-a, c-a) / dis(a, b) / dis(a, c)),
           B = std::acos(dot(c-b, a-b) / dis(b, c) / dis(b, a)),
           C = std::acos(dot(a-c, b-c) / dis(c, a) / dis(c, b));
...
double rCC(const Point &a, const Point &b, const Point &c) {
    return dis(b, c) / 2.0 / std::sin(std::acos(dot(b-a, c-a) / dis(a, b) / dis(a, c)));
}
...
double rIC(const Point &a, const Point &b, const Point &c) {
    return 2.0 * area(a, b, c) / (dis(a, b) + dis(b, c) + dis(c, a));
}
